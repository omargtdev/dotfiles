...
globalkeys = mytable.join(
    ...
    --[[ Screen brightness for laptop keys
    ...
    -- Screen brightness
    awful.key({ modkey }, "F3", function () os.execute("brightnessctl set +10%") end,
              {description = "+10%", group = "hotkeys"}),
    awful.key({ modkey }, "F2", function () os.execute("brightnessctl set 10%-") end,
              {description = "-10%", group = "hotkeys"}),
    ...
    awful.key({ altkey }, "Up",
	    ...
	    --[[
	    os.execute("rm $HOME/result")
	    for k, v in pairs(beautiful) do
		os.execute("echo " .. "\"" .. tostring(k) .. " "  .. tostring(v) .. "\" >> $HOME/result")
	    end
	    ]]--	
        end,
